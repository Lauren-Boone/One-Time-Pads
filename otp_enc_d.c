...
/***********************************************************
Function encdoeMessage
Descript:this function encodes a message using a key
**********************************************************/
void encodeMessage(char msg[], char key[], int size) {
	int i;
	int letter = 'A', c,k;
	for (i = 0; i < size; ++i) {
		c = (int)msg[i];
		...
		//printf("S: key leter is %c\n",key[i]);
	//	printf("S: key letter as int %d\n", k);
		if (c != 10) {
			
			c = changeToCorrectInt(c) + changeToCorrectInt(k);
		//	printf("added letter %d\n", c);
				//c = (msg[i] -letter)+(key[i]-letter);

				//printf("S: letter as int %d\n", c);
			//}
			//else {
			//	c=c+k;
		//	}
...
int main(int argc, char *argv[]) {
	...
	char buffer[256], message[7000], key[7000];
